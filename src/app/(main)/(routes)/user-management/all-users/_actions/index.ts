...
export const getSpecificUser = async (
  ...
  try {
    ...
    // const { token, business_id } = session?.user!
    const { data } = await axios.get(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/customer/${id}`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
...
export const deleteUser = async (id: number, token: string) => {
  try {
    const { data } = await axios.post(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/customer/delete/${id}`,
      null,
      {
        headers: {
          "Content-Type": "application/json",
...
export const editUser = async (
  ...
  try {
    const { data } = await axios.post(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/customer/update/${id}`,
      payload,
      {
        headers: {
          "Content-Type": "application/json",
